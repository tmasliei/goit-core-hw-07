...
class Field:  # Base class of record fields
...
class Name(Field):  # The contact name storage class. Mandatory field.
    def __init__(self, name):
        if not name:
            raise ValueError("Name must not be empty.")
        super().__init__(name)

class Phone(Field):  # Phone number storage class. checks the phone (10 digits).
    def __init__(self, phone):
        if not phone.isdigit() or len(phone) != 10:
            raise ValueError("Phone number must have 10 digits.")
        super().__init__(phone)

class Birthday(Field):  # Date of birth storage class.
    def __init__(self, value):
        try:
...
class Record:  # Class for storing contact information. Name and phone list
    def __init__(self, name):
        self.name = Name(name)
        self.phones = []
        self.birthday = None
...
class AddressBook(UserDict):  # Record storage and management class
            ...
            if user.birthday:
                ...
                if birthday_this_year < today:
                    ...
                    if birthday_this_year.weekday() == 5:  # Субота
                        birthday_this_year += timedelta(days=2)  # Change for Monday
                    elif birthday_this_year.weekday() == 6:  # Неділя
                        birthday_this_year += timedelta(days=1)  # Change for Monday
...
def input_error(func):  # Exception handling decorator function.
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            ...
            if len(args) != 2:
                ...
                continue
            name, phone = args
            if not phone.isdigit() or len(phone) != 10:  # input validation
